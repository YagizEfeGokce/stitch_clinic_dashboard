...
export default function ServiceAnalytics({ appointments }) {
    ...
    const data = appointments.reduce((acc, curr) => {
        const name = curr.services?.name || 'Bilinmeyen';
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 h-full flex flex-col">
            <div className="mb-6">
                <h3 className="font-bold text-slate-900 text-lg">Popüler Tedaviler</h3>
                <p className="text-xs text-slate-500">Gerçekleştirilen hizmetlerin dağılımı</p>
            </div>
